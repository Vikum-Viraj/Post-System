...
const Quotations: React.FC = () => {
  ...
  const handleDeleteQuotation = async (quotationId: string | number) => {
    ...
    setIsDeleting(String(quotationId));
    try {
      await axiosInstance.delete(`/quotations/${quotationId}`);
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      ...
                      <button
                        ...
                        disabled={isDeleting === String(quotation.id)}
                      >
                        {isDeleting === String(quotation.id) ? (
                          <span className="loader mr-1" style={{ width: 16, height: 16, display: 'inline-block', border: '2px solid #ef4444', borderTop: '2px solid transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
                        ) : null}
                        {/* Lucide Trash Icon */}
